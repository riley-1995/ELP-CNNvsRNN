...
positive_samples_data = data[~data['Tag 1'].isin(negative_signal)]
positive_samples_data = positive_samples_data[~positive_samples_data['notes'].str.contains('faint|marginal|gorilla', case=False, na=False)]
...
for file in set(positive_samples_data['Begin File']):
    target_file = os.path.join(data_folder, file)

    if os.path.exists(target_file):
        print(f"Opening file: {target_file}")
        clips = positive_samples_data[positive_samples_data['Begin File'] == file]
