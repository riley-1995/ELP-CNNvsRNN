...
class small_cnn(tf.keras.Model):
	...
	def __init__(self, cfg, training):
		...
		self.pool3 = tf.keras.layers.MaxPooling2D(3, 2, 'VALID')

		# Fully connected layers
		fc_init = tf.compat.v1.glorot_normal_initializer()

		self.fc1 = tf.keras.layers.Dense(512, activation=tf.nn.relu, kernel_initializer=fc_init)
		self.drop1 = tf.keras.layers.Dropout(self.cfg.DROPOUT)

		self.fc2 = tf.keras.layers.Dense(512, activation=tf.nn.relu, kernel_initializer=fc_init)
